...
function drawRpsIcon(ctx, choice, cx, cy, color) {
  ...
  if (choice === 'rock') {
    ...
    ctx.bezierCurveTo(cx - 60, cy - 40, cx - 20, cy - 60, cx + 10, cy - 52);
    ctx.bezierCurveTo(cx + 50, cy - 50, cx + 62, cy - 10, cx + 48, cy + 30);
    ctx.bezierCurveTo(cx + 30, cy + 62, cx - 30, cy + 58, cx - 52, cy + 10);
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = 'rgba(0,0,0,0.18)';
    ctx.beginPath();
    ctx.ellipse(cx + 8, cy + 14, 30, 18, -0.4, 0, Math.PI * 2);
    ctx.fill();
  } else if (choice === 'paper') {
    roundRect(ctx, cx - 44, cy - 58, 88, 116, 10);
    ctx.fill();
    ctx.strokeStyle = 'rgba(0,0,0,0.25)';
    ctx.lineWidth = 4;
    for (let i = 0; i < 4; i++) {
      ctx.beginPath();
      ctx.moveTo(cx - 28, cy - 30 + i * 20);
      ctx.lineTo(cx + (i === 3 ? 4 : 28), cy - 30 + i * 20);
      ctx.stroke();
    }
  } else if (choice === 'scissors') {
    ctx.beginPath();
    ctx.moveTo(cx - 38, cy - 56);
    ...
    ctx.lineWidth = 8;
    ctx.beginPath();
    ctx.arc(cx - 34, cy + 48, 17, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(cx + 34, cy + 48, 17, 0, Math.PI * 2);
    ctx.stroke();
  } else {
    text(ctx, '?', cx, cy + 2, { size: 80, bold: true, color, align: 'center', baseline: 'middle' });
  }
  ctx.restore();
}
